...
class ExamSetController extends Controller
{
    public function index(Request $request)
    {
                    ...
                    $actionBtn = '
                            <button class="btn btn-primary btn-border dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Action</button>
                                    <div class="dropdown-menu">
                                        <a href="'.@route('staff.exam-sets-edit-page',['exam_set_id' => $row->id]).'"  class="dropdown-item">Edit Exam Set</a> 
                                        <a href="'.@route('staff.exam-sets-delete',['exam_set_id' => $row->id]).'" class="dropdown-item confirm_delete">Delete Exam Set</a> 
                                        <a href="'.@route('staff.exam-sets-status',['exam_set_id' => $row->id, 'status' =>$status]).'" class="dropdown-item confirm_status" title="'.$title.'">'.$text.'</a>
                                    </div>';
                    return $actionBtn;
                })
                ->rawColumns(['action'])
        ...
        return view('staff.exams.sets.index', $data);
    }

    public function create(Request $request)
    {
        $data['title'] = 'Create Exam Set';
        return view('staff.exams.sets.create', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        try 
        {
            ...
            return redirect()->route('staff.exam-sets-index-page')->with('success', 'Exam set created successfully');
        } catch (Exception $e) {
            return redirect()->route('staff.exam-sets-index-page')->with('error', 'Oops something went wrong');
        }
    }
    ...
    public function edit(Request $request, $exam_set_id)
    {
        ...
        if(is_null($data['exam_set'])){
            return redirect()->route('staff.exam-sets-index-page')->with('error', 'Exam set not found');
        }
        $data['title'] = 'Edit Exam Set';
        return view('staff.exams.sets.edit', $data);
    }
    ...
    public function update(Request $request, $exam_set_id)
    {
        ...
        try 
        {
            ...
            return redirect()->route('staff.exam-sets-index-page')->with('success', 'Exam set updated successfully');
        } catch (Exception $e) {
            return redirect()->route('staff.exam-sets-index-page')->with('error', 'Oops something went wrong');
        }
    }
    ...
    public function setGradings(Request $request)
    {
        ...
        return view('staff.exams.sets.set_gradings', $data);
    }
    ...
    public function marksConsideration(Request $request)
    {
        ...
        $data['class_lists']  = SubjectClass::with('exam_sets')->where('is_deleted','no')->get();
        $data['title'] = 'Set Gradings';
        return view('staff.exams.sets.marks_consideration', $data);
    }
