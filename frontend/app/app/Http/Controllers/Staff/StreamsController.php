...
class StreamsController extends Controller
{
    public function index(Request $request)
    {
                    ...
                    $actionBtn = '
                            <button class="btn btn-primary btn-border dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Action</button>
                                    <div class="dropdown-menu">
                                        <a href="'.@route('staff.streams-edit-page',['stream_id' => $row->id]).'"  class="dropdown-item">Edit Stream</a> <a href="'.@route('staff.streams-delete',['stream_id' => $row->id]).'" class="dropdown-item confirm_delete">Delete Stream</a> <a href="'.@route('staff.streams-status',['stream_id' => $row->id, 'status' =>$status]).'" class="dropdown-item confirm_status" title="'.$title.'">'.$text.'</a>
                                    </div>';
                    return $actionBtn;
                })
                ->rawColumns(['action'])
        ...
        return view('staff.streams.index', $data);
    }
    ...
    public function create(Request $request)
    {
        ...
        return view('staff.streams.create', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        {
            Stream::create($ins);
            return redirect()->route('staff.streams-page')->with('success', 'Stream created successfully');
        } catch (Exception $e) {
            return redirect()->route('staff.streams-page')->with('error', 'Oops something went wrong');
        }
    }
    ...
    public function edit(Request $request, $stream_id)
    {
        ...
        if(is_null($data['stream'])){
            return redirect()->route('staff.streams-page')->with('error', 'Stream not found');
        }
        $data['classes'] = SubjectClass::where('is_deleted', 'no')->where('class_status', 'active')->get();
        $data['title'] = 'Create Stream';
        return view('staff.streams.edit', $data);
    }
    ...
    public function update(Request $request, $stream_id)
    {
        ...
        try 
        {
            ...
            return redirect()->route('staff.streams-page')->with('success', 'Stream updated successfully');
        } catch (Exception $e) {
            return redirect()->route('staff.streams-page')->with('error', 'Oops something went wrong');
        }
    }
    ...
    public function delete(Request $request, $class_id){
        ...
        try {
            ...
            return redirect()->route('staff.streams-page')->with('success', 'Stream removed successfully');
        } catch (Exception $e) {
            return redirect()->route('staff.streams-page')->with('error', 'Oops something went wrong');
        }
    }
    ...
    public function status(Request $request, $class_id, $status){
        ...
        try {
            Stream::where('id',$class_id)->update($update);
            return redirect()->route('staff.streams-page')->with('success', 'Stream status updated successfully');
        } catch (Exception $e) {
            return redirect()->route('staff.streams-page')->with('error', 'Oops something went wrong');
        }
    }
