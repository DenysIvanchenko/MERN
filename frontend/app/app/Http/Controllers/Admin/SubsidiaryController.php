...
class SubsidiaryController extends Controller
{
    public function index(Request $request)
    {
                    ...
                    $actionBtn = '
                            <button class="btn btn-primary btn-border dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Action</button>
                                    <div class="dropdown-menu">
                                        <a href="'.@route('admin.subject-subsidiaries-edit-page',['subsidiary_id' => $row->id]).'"  class="dropdown-item">Edit Subsidiary</a> 
                                        <a href="'.@route('admin.subject-subsidiaries-delete',['subsidiary_id' => $row->id]).'" class="dropdown-item confirm_delete">Delete Subsidiary</a> 
                                        <a href="'.@route('admin.subject-subsidiaries-status',['subsidiary_id' => $row->id, 'status' =>$status]).'" class="dropdown-item confirm_status" title="'.$title.'">'.$text.'</a>
                                    </div>';
                    return $actionBtn;
                })
                ->rawColumns(['action'])
        ...
        return view('admin.advanced-features.subjects.subsidiaries.index', $data);
    }

    public function create(Request $request)
    {
        $data['title'] = 'Create Subject Paper';
        return view('admin.advanced-features.subjects.subsidiaries.create', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        try 
        {
            ...
            return redirect()->route('admin.subject-subsidiaries-index-page')->with('success', 'Subsidiary added successfully');
        } catch (Exception $e) {
            return redirect()->route('admin.subject-subsidiaries-index-page')->with('error', 'Oops somthing went wrong');
        }
    }
    ...
    public function edit(Request $request, $subsidiary_id)
    {
        ...
        if(is_null($data['subsidiary'])){
            return redirect()->route('admin.subject-subsidiaries-index-page')->with('error', 'Subsidiary not found');
        }
        $data['title'] = 'Edit Subsidiary';
        return view('admin.advanced-features.subjects.subsidiaries.edit', $data);
    }
    ...
    public function update(Request $request, $subsidiary_id)
    {
        ...
        try 
        {
            ...
            return redirect()->route('admin.subject-subsidiaries-index-page')->with('success', 'Subsidiary udpated successfully');
        } catch (Exception $e) {
            return redirect()->route('admin.subject-subsidiaries-index-page')->with('error', 'Oops somthing went wrong');
        }
    }
    ...
    public function delete(Request $request, $subsidiary_id){
        ...
        try {
            ...
            return redirect()->route('admin.subject-subsidiaries-index-page')->with('success', 'Subsidiary removed successfully');
        } catch (Exception $e) {
            return redirect()->route('admin.subject-subsidiaries-index-page')->with('error', 'Oops something went wrong');
        }
    }
    ...
    public function status(Request $request, $subsidiary_id, $status){
        ...
        try {
            SubjectSubsidiary::where('id',$subsidiary_id)->update($update);
            return redirect()->route('admin.subject-subsidiaries-index-page')->with('success', 'Subsidiary status updated successfully');
        } catch (Exception $e) {
            return redirect()->route('admin.subject-subsidiaries-index-page')->with('error', 'Oops something went wrong');
        }
    }
