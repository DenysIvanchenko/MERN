...
class ExamController extends Controller
{
    ...
    public function getStaffSubjectPapersCombinations(Request $request)
    {
        ...
        $paper_return = '<option value="">Select</option>';
        if($paper_lists->count() > 0){
            foreach ($paper_lists as $value) {
                $paper_return.='<option value="'.$value->id.'">Paper '.$value->paper_name.'</option>';
            }
        }
        ...
        if($level == "advanced_level"){
            ...
            $combination_return = '<option value="">Select</option>';
            if($combination_lists->count() > 0){
                foreach ($combination_lists as $value) {
                    $combination_return.='<option value="'.$value->id.'">'.$value->combination_name.'</option>';
                }
            }
            return response()->json([
                'paper_count' => $paper_lists->count(),
                'paper_options' => $paper_return,
                'combination_count' => $combination_lists->count(),
                'combination_options' => $combination_return,
            ],200);
        }else{
            return response()->json([
                'paper_count' => $paper_lists->count(),
