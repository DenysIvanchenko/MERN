...
class SubjectGradingController extends Controller
{
    public function index(Request $request)
    {
                    ...
                    $actionBtn = '
                            <button class="btn btn-primary btn-border dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Action</button>
                                    <div class="dropdown-menu">
                                        <a href="'.@route('admin.exams-gradings-edit-page',['grade_id' => $row->id]).'"  class="dropdown-item">Edit Grade</a> <a href="'.@route('admin.exams-gradings-delete',['grade_id' => $row->id]).'" class="dropdown-item confirm_delete">Delete Grade</a> <a href="'.@route('admin.exams-gradings-status',['grade_id' => $row->id, 'status' =>$status]).'" class="dropdown-item confirm_status" title="'.$title.'">'.$text.'</a>
                                    </div>';
                    return $actionBtn;
                })
                ->rawColumns(['action'])
        ...
        return view('admin.exams.gradings.index', $data);
    }

    public function create(Request $request)
    {
        $data['title'] = 'Create Grade';
        return view('admin.exams.gradings.create', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        try 
        {
            ...
            return redirect()->route('admin.exams-gradings-index-page')->with('success', 'Grading added successfully');
        } catch (Exception $e) {
            return redirect()->route('admin.exams-gradings-index-page')->with('error', 'Oops something went wrong');
        }
    }
    ...
    public function edit(Request $request, $grade_id)
    {
        ...
        if(is_null($data['grading'])){
            return redirect()->route('admin.exams-gradings-index-page')->with('error', 'Grading not found');
        }
        $data['title'] = 'Create Grade';
        return view('admin.exams.gradings.edit', $data);
    }
    ...
    public function update(Request $request, $grade_id)
    {
        ...
        try 
        {
            ...
            return redirect()->route('admin.exams-gradings-index-page')->with('success', 'Grading updated successfully');
        } catch (Exception $e) {
            return redirect()->route('admin.exams-gradings-index-page')->with('error', 'Oops something went wrong');
        }
    }
    ...
    public function delete(Request $request, $grade_id){
        ...
        try {
            ...
            return redirect()->route('admin.exams-gradings-index-page')->with('success', 'Grading removed successfully');
        } catch (Exception $e) {
            return redirect()->route('admin.exams-gradings-index-page')->with('error', 'Oops something went wrong');
        }
    }
    ...
    public function status(Request $request, $grade_id, $status){
        ...
        try {
            SubjectGrading::where('id',$grade_id)->update($update);
            return redirect()->route('admin.exams-gradings-index-page')->with('success', 'Grading status updated successfully');
        } catch (Exception $e) {
            return redirect()->route('admin.exams-gradings-index-page')->with('error', 'Oops something went wrong');
        }
    }
